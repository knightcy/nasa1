...
export default function BoneDensityInteractive() {
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto my-8 border-[var(--nasa-bg-hover)] bg-[var(--nasa-bg-secondary)]">
      <CardHeader>
        <CardTitle className="text-[var(--nasa-text)]">微重力下的骨密度流失</CardTitle>
      </CardHeader>
      <CardContent className="space-y-8">
        <div className="relative h-80 bg-[var(--nasa-bg-tertiary)] rounded-lg overflow-hidden">
          ...
          <div className="absolute top-4 left-4 z-10 pointer-events-none">
            <div className="bg-[var(--nasa-bg-tertiary)]/80 px-4 py-2 rounded-md font-bold text-lg shadow-sm text-[var(--nasa-text)] border border-[var(--nasa-bg-hover)]">
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="font-medium text-[var(--nasa-text)]">在太空停留时间: {months[0]} 个月</label>
            </div>
            <Slider 
              value={months} 
              ...
              className="w-full"
            />
            <p className="text-xs text-[var(--nasa-text-secondary)]">拖动滑块模拟宇航员在微重力环境下的停留时间（假设无阻力锻炼）。</p>
          </div>
